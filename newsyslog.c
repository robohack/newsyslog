	...
	struct conf_entry *ent;
{
	...
	if (might_need_newlog && !(ent->flags & CE_NOCREATE)) {
		if (verbose) {
			printf("# %screating %s with owner %d:%d, mode 0%03o\n",
			       create_only ? "(re)" : "",
			       ent->log, ent->uid, ent->gid, ent->permissions);
		}
		/*
		 * When creating the new log file use a unique name generated by
		 * mkstemp() so that there's no race condition between when it
		 * appears in the filesystem under its final name and when it
		 * gains the correct ownership and permissions.  This prevents
		 * some daemons from accidentally trying to use a file they may
		 * not have permission yet to use.
		 *
		 * Note:  Having the CE_NOCREATE flag set correctly for whether
		 * or not the owner daemon creates the file itself is necessary
		 * to prevent spurious warnings from either newsyslog or the
		 * daemon should they race to create the file.
		 */
		if (show_script) {
